...
export class VideoRecognizerComponent implements AfterViewInit, OnDestroy {
  ...
  updateMessages(message: IMessage) {
    if (!this.isNumber(message.progress))
    ...
    const last: IMessage | null = (this.messages.length ? this.messages[this.messages.length - 1] : null) || null;
  ...
  isNumber(x: number | null | undefined): x is number {
